...
class BaseBotLogic
  ...
  def self.reply_message(msg, options={})
    ...
    if @request_type == "TEXT" or @request_type == "CALLBACK"  or @request_type == "LOCATION" or @request_type == "IMAGE"
  ...
  def self.reply_location_button(msg)
    if @request_type == "TEXT" or @request_type == "IMAGE" or @request_type == "CALLBACK"
      msg = compute_emojis(msg)
      Bot.deliver(
        recipient: {id: @current_user.fb_id},
        message:{
  ...
  #geo utils
  def self.get_address_from_latlng(location)
    #https://console.developers.google.com/flows/enableapi?apiid=geolocation&keyType=SERVER_SIDE&reusekey=true
    response = HTTParty.get("https://maps.googleapis.com/maps/api/geocode/json?latlng=#{location["coordinates"]["lat"]},#{location["coordinates"]["long"]}&key=#{ENV["GEOCODING_API"]}")
